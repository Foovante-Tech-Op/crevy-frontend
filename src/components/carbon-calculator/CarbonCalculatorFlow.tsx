...
// Shared carbon footprint calculator flow.
//
// This used to live only at (dashboard)/carbon-calculator/page.tsx. Per
// product decision, field agents now get the exact same 4-step form
// (household → transportation → energy & consumption → results/history)
// instead of a separate, simplified travel-only version — same fields,
// same validation, same results/history UI, same backend endpoint
// (`POST /carbon-footprint/calculate` + `GET /carbon-footprint/history`).
// Extracted here as a single source of truth so both
// (dashboard)/carbon-calculator/page.tsx and (agent)/agent/calculator/page.tsx
// can render it verbatim rather than maintaining two forms that drift apart.
...
/**
 * CarbonCalculatorFlow — the full 4-step form + results/history view.
 *
 * Single source of truth for the calculator UI, rendered as-is by both
 * the admin dashboard route and the field-agent app route so the two
 * surfaces stay identical rather than drifting into two different forms.
 */
...
export default CarbonCalculatorFlow;
